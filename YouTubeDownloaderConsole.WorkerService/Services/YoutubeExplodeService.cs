...
namespace YouTubeDownloaderConsole.WorkerService.Services
{
    public class YoutubeExplodeService
    {
        ...
        public async Task DownloadVideoAsync(string url, string outputFolder, string ffmpegPath)
        {
            try
            {
                ...
                await _youtube.Videos.DownloadAsync(url, filePathToSave, o => o
                    .SetContainer("mp4")
        ...
        /// <param name="ffmpegPath">The path to the ffmpeg executable used for video conversion.</param>
        public async Task DownloadPlaylistAsync(string playlistUrl, string outputFolder, string ffmpegPath)
        {
            try
            {
                ...
                foreach (var video in videos)
                {
                    ...
                    await DownloadVideoAsync(video.Url, outputFolder, ffmpegPath);
                }
            }
            catch (Exception ex)
            {
